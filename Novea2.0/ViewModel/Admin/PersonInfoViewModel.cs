...
namespace Novea2._0.ViewModel.Admin
{
    public class PersonInfoViewModel : BaseViewModel
    {
        ...
        void UpdateInfo(PersonInfo p)
        {
            ...
            int dem2 = DataProvider.Ins.DB.CUAHANGs.Where(k => k.TAIKHOAN == p.tbTENDANGNHAP.Text).Count();
            int dem3 = DataProvider.Ins.DB.SHIPPERs.Where(k => k.TAIKHOAN == p.tbTENDANGNHAP.Text).Count();
            int dem4 = DataProvider.Ins.DB.ADMINIS.Where(k => k.TAIKHOAN == p.tbTENDANGNHAP.Text).Count();
            if ((dem1 > 0 || dem2 > 0 || dem3> 0) || (dem4 > 0 && p.tbTENDANGNHAP.Text != Const.ADM.TAIKHOAN))
            {
                MessageBox.Show("Tên đăng nhập đã tồn tại !", "THÔNG BÁO", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            int dem5 = DataProvider.Ins.DB.KHACHes.Where(k => k.EMAIL == p.tbMAIL.Text).Count();
            int dem6 = DataProvider.Ins.DB.CUAHANGs.Where(k => k.EMAIL == p.tbMAIL.Text).Count();
            int dem7 = DataProvider.Ins.DB.SHIPPERs.Where(k => k.EMAIL == p.tbMAIL.Text).Count();
            int dem8 = DataProvider.Ins.DB.ADMINIS.Where(k => k.EMAIL == p.tbMAIL.Text).Count();
            if ((dem5 > 0 || dem6 > 0 || dem7 > 0) || (dem8 > 0 && p.tbMAIL.Text != Const.ADM.EMAIL))
            {
            ...
            var temp = DataProvider.Ins.DB.ADMINIS.Where(k => k.MAND == Const.ADM.MAND).FirstOrDefault();
            ...
            bitmapImage.StreamSource = new MemoryStream(Const.ADM.AVATAR);
            bitmapImage.EndInit();
            MainWindow.Instance.image.ImageSource = bitmapImage;
            MainWindow.Instance.tbHoten.Text = string.Join(" ", Const.ADM.HOTEN.Split().Reverse().Take(2).Reverse());
        }
        void UpdateAvatar()
        {
        ...
        void LoadPersonInfowd(PersonInfo p)
        {
            ...
            bitmapImage.EndInit();
            Avatar = bitmapImage;
            p.tbTENDANGNHAP.Text = Const.ADM.TAIKHOAN;
            p.cbbGIOITINH.Text = Const.ADM.GIOITINH;
            p.tbHOTEN.Text = Const.ADM.HOTEN;
            p.NS.SelectedDate = Const.ADM.NGSINH;
            p.tbSDT.Text = Const.ADM.SDT;
            p.tbMAIL.Text = Const.ADM.EMAIL;
        }
    }
}
