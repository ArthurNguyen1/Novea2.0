...
namespace Novea2._0.ViewModel.Login
{
    public class AdminSignUpViewModel : BaseViewModel
    {
        ...
        {
            MoveWdCommand = new RelayCommand<AdminSignUp>((p) => true, (p) => MoveWindow(p));
            MinimizeWdCommand = new RelayCommand<AdminSignUp>((p) => true, (p) => MinimizeWindow(p));
            CloseWdCommand = new RelayCommand<AdminSignUp>((p) => true, (p) => CloseWindow(p));
            AddImageCommand = new RelayCommand<ImageBrush>((p) => true, (p) => AddImage());
            SignUpCommand = new RelayCommand<AdminSignUp>((p) => true, (p) => SignUp(p));
        }
        void SignUp(AdminSignUp p)
        {
            ...
            if (h == MessageBoxResult.Yes)
            {
                ...
                temp.STATU = true;
                temp.REASONBANNING = "";
                DataProvider.Ins.DB.ADMINIS.Add(temp);
        ...
        string rdMAADM()
        {
            ...
            do
            {
                ...
                MaADM = "ADM" + rand.Next(0, 10000).ToString();
            } while (checkMAADM(MaADM));
            return MaADM;
        }
        void AddImage()
        {
        ...
        void CloseWindow(AdminSignUp p)
        {
            p.Close();
        }
        void MinimizeWindow(AdminSignUp p)
        {
            p.WindowState = WindowState.Minimized;
        }
        void MoveWindow(AdminSignUp p)
        {
            p.DragMove();
        }
    }
