...
namespace Novea2._0.ViewModel.Login
{
    public class StoreLoginViewModel : BaseViewModel
    {
        ...
        public StoreLoginViewModel()
        {
            ...
            RegisterCommand = new RelayCommand<StoreLogin>((p) => true, (p) => Register(p));
            ForgetPassCommand = new RelayCommand<StoreLogin>((p) => true, (p) => ForgetPass(p));
        }
        void loadwd()
        {
            Const.IsLogin = false;
        ...
        void Register(StoreLogin parameter)
        {
            StoreSignUp storeSignUp = new StoreSignUp();
            storeSignUp.ShowDialog();
        }
        void ForgetPass(StoreLogin parameter)
        {
