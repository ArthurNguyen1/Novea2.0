...
{
    public class ShipperSignUpViewModel : BaseViewModel
    {
        ...
        public ShipperSignUpViewModel()
        {
            MoveWdCommand = new RelayCommand<ShipperSignUp>((p) => true, (p) => MoveWindow(p));
            MinimizeWdCommand = new RelayCommand<ShipperSignUp>((p) => true, (p) => MinimizeWindow(p));
            CloseWdCommand = new RelayCommand<ShipperSignUp>((p) => true, (p) => CloseWindow(p));
            AddImageCommand = new RelayCommand<ImageBrush>((p) => true, (p) => AddImage());
            SignUpCommand = new RelayCommand<ShipperSignUp>((p) => true, (p) => SignUp(p));
        }
        void SignUp(ShipperSignUp p)
        {
            ...
            if (h == MessageBoxResult.Yes)
            {
                ...
                temp.LUONG = 0;
                temp.SHIPSTATUS = false;
                temp.STATU = true;
                temp.REASONBANNING = "";
                DataProvider.Ins.DB.SHIPPERs.Add(temp);
        ...
        string rdMASHIP()
        {
            ...
            do
            {
                ...
                MaSHIP = "SHIP" + rand.Next(0, 10000).ToString();
            } while (checkMASHIP(MaSHIP));
            return MaSHIP;
        }
        void AddImage()
        {
        ...
        void CloseWindow(ShipperSignUp p)
        {
            p.Close();
        }
        void MinimizeWindow(ShipperSignUp p)
        {
            p.WindowState = WindowState.Minimized;
        }
        void MoveWindow(ShipperSignUp p)
        {
            p.DragMove();
        }
    }
